...
$langs_add = NULL; // Initialize page object first

class clangs_add extends clangs {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["lang_id"] != "") {
				...
				$this->setKey("lang_id", $this->lang_id->CurrentValue); // Set up key
			} else {
				$this->setKey("lang_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$loaded) { // Record not loaded
					...
					$this->Page_Terminate("langslist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "langsview.php")
...
// Create page object
if (!isset($langs_add)) $langs_add = new clangs_add();
...
var CurrentForm = flangsadd = new ew_Form("flangsadd", "add");

// Validate form
flangsadd.Validate = function() {
...
flangsadd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $langs_add->ShowPageHeader(); ?>
<?php
$langs_add->ShowMessage();
?>
<form name="flangsadd" id="flangsadd" class="<?php echo $langs_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($langs_add->CheckToken) { ?>
