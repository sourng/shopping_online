				...
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->cat_id->CurrentValue) == strval($this->Recordset->fields('cat_id'))) {
...
var CurrentForm = fcategoriesview = new ew_Form("fcategoriesview", "view");
...
fcategoriesview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fcategoriesview.Lists["x_cat_home[]"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcategoriesview.Lists["x_cat_home[]"].Options = <?php echo json_encode($categories_view->cat_home->Options()) ?>;
...
<?php if (!isset($categories_view->Pager)) $categories_view->Pager = new cPrevNextPager($categories_view->StartRec, $categories_view->DisplayRecs, $categories_view->TotalRecs, $categories_view->AutoHidePager) ?>
...
<!--first page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $categories_view->PageUrl() ?>start=<?php echo $categories_view->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($categories_view->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $categories_view->PageUrl() ?>start=<?php echo $categories_view->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $categories_view->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($categories_view->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $categories_view->PageUrl() ?>start=<?php echo $categories_view->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($categories_view->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $categories_view->PageUrl() ?>start=<?php echo $categories_view->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<?php } ?>
<form name="fcategoriesview" id="fcategoriesview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($categories_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $categories_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="categories">
<input type="hidden" name="modal" value="<?php echo intval($categories_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($categories->cat_id->Visible) { // cat_id ?>
	<tr id="r_cat_id">
		<td class="col-sm-2"><span id="elh_categories_cat_id"><?php echo $categories->cat_id->FldCaption() ?></span></td>
...
<?php if ($categories->cat_name->Visible) { // cat_name ?>
	<tr id="r_cat_name">
		<td class="col-sm-2"><span id="elh_categories_cat_name"><?php echo $categories->cat_name->FldCaption() ?></span></td>
...
<?php if ($categories->cat_ico_class->Visible) { // cat_ico_class ?>
	<tr id="r_cat_ico_class">
		<td class="col-sm-2"><span id="elh_categories_cat_ico_class"><?php echo $categories->cat_ico_class->FldCaption() ?></span></td>
...
<?php echo $categories->cat_ico_class->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($categories->cat_ico_image->Visible) { // cat_ico_image ?>
	<tr id="r_cat_ico_image">
...
<?php if ($categories->cat_home->Visible) { // cat_home ?>
	<tr id="r_cat_home">
		<td class="col-sm-2"><span id="elh_categories_cat_home"><?php echo $categories->cat_home->FldCaption() ?></span></td>
