...
$product_gallery_update = NULL; // Initialize page object first

class cproduct_gallery_update extends cproduct_gallery {
	...
	// Page object name
	var $PageObjName = 'product_gallery_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("product_gallerylist.php"); // No records selected, return to list
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					if (!ew_CompareValue($this->product_id->DbValue, $this->Recordset->fields('product_id')))
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$this->pro_gallery_id->CurrentValue = $sKeyFld;
		$sKeyFld = $key[1];
		if (!is_numeric($sKeyFld))
			return FALSE;
	...
	function GetUploadFiles() {
		...
		$this->thumnail->MultiUpdate = $objForm->GetValue("u_thumnail");
	...
	function LoadFormValues() {
		...
		$this->product_id->MultiUpdate = $objForm->GetValue("u_product_id");
		if (!$this->pro_gallery_id->FldIsDetailKey)
	...
	function ValidateForm() {
		...
		$gsFormError = "";
		$lUpdateCnt = 0;
		if ($this->product_id->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->thumnail->MultiUpdate == "1") $lUpdateCnt++;
...
// Create page object
if (!isset($product_gallery_update)) $product_gallery_update = new cproduct_gallery_update();

// Page init
$product_gallery_update->Page_Init();

// Page main
$product_gallery_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$product_gallery_update->Page_Render();
?>
...
var CurrentForm = fproduct_galleryupdate = new ew_Form("fproduct_galleryupdate", "update");

// Validate form
fproduct_galleryupdate.Validate = function() {
...
fproduct_galleryupdate.Form_CustomValidate = 
...
fproduct_galleryupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fproduct_galleryupdate.Lists["x_product_id"] = {"LinkField":"x_product_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_pro_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"products"};
fproduct_galleryupdate.Lists["x_product_id"].Data = "<?php echo $product_gallery_update->product_id->LookupFilterQuery(FALSE, "update") ?>";
...
<?php $product_gallery_update->ShowPageHeader(); ?>
<?php
$product_gallery_update->ShowMessage();
?>
<form name="fproduct_galleryupdate" id="fproduct_galleryupdate" class="<?php echo $product_gallery_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($product_gallery_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $product_gallery_update->Token ?>">
<?php } ?>
...
<?php } ?>
<input type="hidden" name="modal" value="<?php echo intval($product_gallery_update->IsModal) ?>">
<?php foreach ($product_gallery_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_product_galleryupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"<?php echo $product_gallery_update->Disabled ?>> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($product_gallery->thumnail->Visible) { // thumnail ?>
	<div id="r_thumnail" class="form-group">
		<label class="<?php echo $product_gallery_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($product_gallery->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_thumnail" id="u_thumnail" class="ewMultiSelect" value="1"<?php echo ($product_gallery->thumnail->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($product_gallery->thumnail->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_thumnail" id="u_thumnail" value="<?php echo $product_gallery->thumnail->MultiUpdate ?>">
<?php } ?>
 <?php echo $product_gallery->thumnail->FldCaption() ?></label></div></label>
		<div class="<?php echo $product_gallery_update->RightColumnClass ?>"><div<?php echo $product_gallery->thumnail->CellAttributes() ?>>
...
<?php if (@$_POST["fa_x_thumnail"] == "0") { ?>
<input type="hidden" name="fa_x_thumnail" id= "fa_x_thumnail" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_thumnail" id= "fa_x_thumnail" value="1">
<?php } ?>
...
<?php if ($product_gallery->image->Visible) { // image ?>
	<div id="r_image" class="form-group">
		<label class="<?php echo $product_gallery_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($product_gallery->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_image" id="u_image" class="ewMultiSelect" value="1"<?php echo ($product_gallery->image->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($product_gallery->image->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_image" id="u_image" value="<?php echo $product_gallery->image->MultiUpdate ?>">
<?php } ?>
 <?php echo $product_gallery->image->FldCaption() ?></label></div></label>
		<div class="<?php echo $product_gallery_update->RightColumnClass ?>"><div<?php echo $product_gallery->image->CellAttributes() ?>>
...
<?php if (!$product_gallery_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $product_gallery_update->OffsetColumnClass ?>"><!-- buttons offset -->
<?php if ($product_gallery->CurrentAction <> "F") { // Confirm page ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $product_gallery_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
<?php } else { ?>
...
<script type="text/javascript">
fproduct_galleryupdate.Init();
</script>
<?php
$product_gallery_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$product_gallery_update->Page_Terminate();
?>
