...
$product_gallery_grid = NULL; // Initialize page object first

class cproduct_gallery_grid extends cproduct_gallery {
	...
	// Page object name
	var $PageObjName = 'product_gallery_grid';

	// Grid form hidden field names
	var $FormName = 'fproduct_gallerygrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["product_gallery"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadDefaultValues() {
		...
		$this->thumnail->Upload->Index = $this->RowIndex;
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				$this->pro_gallery_id->CurrentValue = strval($arKeys[0]); // pro_gallery_id
			else
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->product_id->CurrentValue = strval($arKeys[1]); // product_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "products") {
			...
			if ($GLOBALS["products"]->EventCancelled) $this->EventCancelled = TRUE;
		}
