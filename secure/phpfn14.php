...
function CurrentProjectID() {
	...
	return "{F38FCD2E-3638-4192-9CEA-A2A8E31F8537}";
}
	...
	function ValidateUser(&$usr, &$pwd, $autologin, $encrypted = FALSE, $provider = "") {
		...
		if (!$valid) {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$valid = $customValid || ew_ComparePassword($rs->fields('com_password'), $pwd, $encrypted);
					if ($valid) {
						$_SESSION[EW_SESSION_STATUS] = "login";
						$_SESSION[EW_SESSION_SYS_ADMIN] = 0; // Non System Administrator
						$this->setCurrentUserName($rs->fields('com_email')); // Load user name

						// Call User Validated event
						$row = $rs->fields;
						$UserProfile->Assign($row);
						$UserProfile->Delete('com_password'); // Delete password
