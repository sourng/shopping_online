...
//
// Table class for tbl_pages
//
class ctbl_pages extends cTable {
	var $page_id;
	var $page_name;
	var $page_title;
	var $page_url;
	...
	function __construct() {
		...
		$this->TableVar = 'tbl_pages';
		$this->TableName = 'tbl_pages';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tbl_pages`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// page_id
		$this->page_id = new cField('tbl_pages', 'tbl_pages', 'x_page_id', 'page_id', '`page_id`', '`page_id`', 3, -1, FALSE, '`page_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// page_name
		$this->page_name = new cField('tbl_pages', 'tbl_pages', 'x_page_name', 'page_name', '`page_name`', '`page_name`', 200, -1, FALSE, '`page_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->page_name->Sortable = TRUE; // Allow sort
		$this->fields['page_name'] = &$this->page_name;

		// page_title
		$this->page_title = new cField('tbl_pages', 'tbl_pages', 'x_page_title', 'page_title', '`page_title`', '`page_title`', 200, -1, FALSE, '`page_title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->page_title->Sortable = TRUE; // Allow sort
		$this->fields['page_title'] = &$this->page_title;

		// page_url
		$this->page_url = new cField('tbl_pages', 'tbl_pages', 'x_page_url', 'page_url', '`page_url`', '`page_url`', 200, -1, FALSE, '`page_url`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->page_url->Sortable = TRUE; // Allow sort
		$this->fields['page_url'] = &$this->page_url;

		// page_description
		$this->page_description = new cField('tbl_pages', 'tbl_pages', 'x_page_description', 'page_description', '`page_description`', '`page_description`', 200, -1, FALSE, '`page_description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->page_description->Sortable = TRUE; // Allow sort
		$this->fields['page_description'] = &$this->page_description;

		// page_detail
		$this->page_detail = new cField('tbl_pages', 'tbl_pages', 'x_page_detail', 'page_detail', '`page_detail`', '`page_detail`', 201, -1, FALSE, '`page_detail`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->page_detail->Sortable = TRUE; // Allow sort
		$this->fields['page_detail'] = &$this->page_detail;

		// page_icon
		$this->page_icon = new cField('tbl_pages', 'tbl_pages', 'x_page_icon', 'page_icon', '`page_icon`', '`page_icon`', 200, -1, FALSE, '`page_icon`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->page_icon->Sortable = TRUE; // Allow sort
		$this->fields['page_icon'] = &$this->page_icon;

		// lang
		$this->lang = new cField('tbl_pages', 'tbl_pages', 'x_lang', 'lang', '`lang`', '`lang`', 200, -1, FALSE, '`lang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tbl_pages`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "tbl_pagesedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "tbl_pagesadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tbl_pagesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tbl_pagesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "tbl_pagesadd.php?" . $this->UrlParm($parm);
		else
			$url = "tbl_pagesadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("tbl_pagesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("tbl_pagesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tbl_pagesdelete.php", $this->UrlParm());
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["page_id"]))
				$arKeys[] = $_POST["page_id"];
	...
	function LoadListRowValues(&$rs) {
		...
		$this->page_description->setDbValue($rs->fields('page_description'));
		$this->page_detail->setDbValue($rs->fields('page_detail'));
		$this->page_icon->setDbValue($rs->fields('page_icon'));
	...
	function RenderEditRow() {
		...
		$this->page_description->EditValue = $this->page_description->CurrentValue;
		...
		$this->page_detail->EditValue = $this->page_detail->CurrentValue;
		...
		$this->page_icon->EditValue = $this->page_icon->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->page_description->Exportable) $Doc->ExportCaption($this->page_description);
					if ($this->page_detail->Exportable) $Doc->ExportCaption($this->page_detail);
					if ($this->page_icon->Exportable) $Doc->ExportCaption($this->page_icon);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->page_description->Exportable) $Doc->ExportField($this->page_description);
						if ($this->page_detail->Exportable) $Doc->ExportField($this->page_detail);
						if ($this->page_icon->Exportable) $Doc->ExportField($this->page_icon);
