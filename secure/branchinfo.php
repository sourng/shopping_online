...
class cbranch extends cTable {
	...
	function __construct() {
		...
		// branch_id
		$this->branch_id = new cField('branch', 'branch', 'x_branch_id', 'branch_id', '`branch_id`', '`branch_id`', 3, -1, FALSE, '`branch_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->branch_id->Sortable = TRUE; // Allow sort
		$this->branch_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['branch_id'] = &$this->branch_id;

		// name
		$this->name = new cField('branch', 'branch', 'x_name', 'name', '`name`', '`name`', 200, -1, FALSE, '`name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->name->Sortable = TRUE; // Allow sort
		$this->fields['name'] = &$this->name;

		// image
		$this->image = new cField('branch', 'branch', 'x_image', 'image', '`image`', '`image`', 200, -1, FALSE, '`image`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`branch`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('branch_id', $rs))
				ew_AddFilter($where, ew_QuotedName('branch_id', $this->DBID) . '=' . ew_QuotedValue($rs['branch_id'], $this->branch_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->branch_id->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->branch_id->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@branch_id@", ew_AdjustSql($this->branch_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("branchview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("branchview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "branchadd.php?" . $this->UrlParm($parm);
		else
			$url = "branchadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("branchedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("branchadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("branchdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "branch_id:" . ew_VarToJson($this->branch_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->branch_id->CurrentValue)) {
			$sUrl .= "branch_id=" . urlencode($this->branch_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["branch_id"]))
				$arKeys[] = $_POST["branch_id"];
			elseif (isset($_GET["branch_id"]))
				$arKeys[] = $_GET["branch_id"];
			else
